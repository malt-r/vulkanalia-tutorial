...
pub(crate) unsafe fn create_image_view(
    ...
    create_image_view_with_components(device, image, format, components.build(), aspects)
}
...
pub(crate) unsafe fn create_depth_objects(
    ...
    // depth attachment should have the same dimensions as the color attachment
    // (defined by swapchain extent), image usage appropriate for depth attachment,
    // optimal tiling and device local memory
    //
    // only question: which format to use?
    // - format must contain a depth component, indicated by Dxx_
    // - we won't be directly accessing the texels from the program, so the format
    //   does not have to an exact one, only needs reasonable accuracy (usually
    //   24 bits in real world applications)
...
unsafe fn get_supported_format(
    ...
    candidates
        ...
        .cloned()
        .find(|f| {
            // support of a format depends on the tiling mode and the usage
            let properties =
                instance.get_physical_device_format_properties(data.physical_device, *f);

            // vk::FormatProperties contains three fields
            ...
            // - buffer_features - use cases that are supported for buffers (not relevant for us here)
            match tiling {
